...
import { parseMentions } from './mention-input';
...
// File-modifying tool names for diff extraction
...
export function ChatInterface() {
    ...
    for (const msg of messages) {
      ...
      const msgToolCalls = parts
        ...
        .map((p: any) => {
          ...
          // Extract old content from DIFF_OLD marker for file-modifying tools
          const isFileTool = FILE_CHANGE_TOOLS.includes(toolName);
          let extractedOldContent: string | undefined;
          let extractedNewContent: string | undefined;
          if (isFileTool && output) {
            const diffOldMatch = output.match(/<!--DIFF_OLD\n([\s\S]*?)\n-->/);
            if (diffOldMatch) {
              extractedOldContent = diffOldMatch[1];
            }
          }
          if (isFileTool) {
            extractedNewContent = String(p.input?.content || p.args?.content || '');
          }
          ...
          return {
            ...
            oldContent: extractedOldContent,
  ...
  // Extract approval request cards from parts
  const approvalCards = (() => {
    const cards: Array<{
      toolName: string;
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Append mention references as context
    if (mentions.length > 0) {
      const mentionContext = mentions.map(m => {
        if (m.type === 'file') return `[引用文件: ${m.label}]`;
        if (m.type === 'conversation') return `[引用对话: ${m.label}]`;
        if (m.type === 'knowledge') return `[引用知识库: ${m.label}]`;
        return '';
      }).filter(Boolean).join('\n');
      if (mentionContext) {
        content = content + '\n\n' + mentionContext;
    ...
    <div className="flex h-full relative">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <ChatMessages
          ...
          CHAT_MODES={CHAT_MODES}
          onStarterPromptClick={(prompt: string) => setInput(prompt)}
        />
