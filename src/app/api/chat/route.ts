...
import { createConversation, createMessage, updateConversation, getApiKeyByProvider, getModelConfig, getSetting, setConversationUserId, getActiveProjectContexts } from '@/lib/db';
...
import { smartTrimMessages } from '@/lib/context-manager';
// === Auto-verification Tool ===
import { verifyTool } from '@/lib/verify-tool';
import { convertMarkdown, readConvertedFile, cleanupConvertedFile } from '@/lib/document-converter';
// === Rate Limiting ===
...
function validateFilePath(filePath: string): string {
  const resolved = require('path').resolve(PROJECT_DIR, filePath);
...
const _DEFAULT_DANGEROUS_PATTERNS = [
  /\brm\s+(-[a-zA-Z]*[rf][a-zA-Z]*\s+)*\/(etc|usr|bin|sbin|lib|boot|dev|proc|sys)\b/,
  /\bdd\b.*\bof=\/dev\//, /\bmkfs\b/, /\bformat\b/,
  /\bchmod\s+777\b/,
  /\bcurl\b.*\|\s*(bash|sh|zsh)\b/, /\bwget\b.*\|\s*(bash|sh|zsh)\b/,
  /\bshutdown\b/, /\breboot\b/, /\binit\s+[0-6]\b/,
  /\biptables\s+-F\b/, /\biptables\s+-X\b/,
  /\b:(){ :\|:& };:/,
  /\b>\/dev\/sda\b/, /\b>\/dev\/nvme/,
  /\bpasswd\b/, /\buseradd\b/, /\buserdel\b/,
  /\bsystemctl\s+(stop|disable|mask)\b/,
  /\bkill\s+-9\s+1\b/,
  /\bnc\b.*\s+-[a-zA-Z]*[lep]/, /\bncat\b.*\s+-[a-zA-Z]*[lep]/,
  /\bsocat\b.*\bEXEC\b/,
]
// Load dangerous patterns from DB, fallback to defaults
let DANGEROUS_CMD_PATTERNS = _DEFAULT_DANGEROUS_PATTERNS;
try { const secRaw = await getSetting('security_config'); if (secRaw) { const sec = JSON.parse(secRaw); if (sec.patterns && Array.isArray(sec.patterns)) DANGEROUS_CMD_PATTERNS = sec.patterns.map((p: string) => new RegExp(p)); } } catch {};
...
function checkDangerousCommand(command: string): { blocked: boolean; reason?: string } {
  ...
  for (const pattern of DANGEROUS_CMD_PATTERNS) {
    if (pattern.test(cmd)) {
      return { blocked: true, reason: 'Dangerous command pattern: ' + pattern.source.substring(0, 60) };
    }
  }
...
async function execRunCommand(command: string): Promise<string> {
  ...
  if (dangerCheck.blocked) {
    return 'BLOCKED: ' + dangerCheck.reason + '. Command blocked by security policy.';
  }
...
import { DEFAULT_MODE_TOOLS, DEFAULT_MODEL_IDENTITY, DEFAULT_TOOL_NAME_ZH, DEFAULT_PROVIDER_MAX_TOKENS, DEFAULT_RUNTIME_CONFIG, DEFAULT_TOOL_SAFETY_TIERS, DEFAULT_SSH_SAFE_COMMANDS, loadToolSafetyTiers, loadSshSafeCommands } from '@/lib/config-defaults';
...
// MODE_TOOLS: loaded from DB at runtime, fallback to defaults
...
// 模型身份映射
// MODEL_IDENTITY: loaded from DB, fallback to defaults
...
export async function POST(request: NextRequest) {
  try {
    // === Rate Limiting ===
    let _rlUserId = 'anonymous';
    try { const _u = await getCurrentUser(); if (_u) _rlUserId = _u.id || _u.email || 'anonymous'; } catch {}
    if (!_rlUserId || _rlUserId === 'anonymous') { _rlUserId = request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip') || 'anonymous'; }
    const _rlResult = await checkRateLimit(_rlUserId);
    if (!_rlResult.allowed) {
      return new Response(JSON.stringify({ error: '请求过于频繁，请稍后再试', retryAfter: _rlResult.retryAfter }), { status: 429, headers: { 'Retry-After': String(_rlResult.retryAfter), 'Content-Type': 'application/json' } });
    }
    ...
    if (model_id === 'auto') {
      try {
        ...
        const userMsg = messages.filter((m: any) => m.role === 'user').map((m: any) => typeof m.content === 'string' ? m.content : '').join(' ');
    ...
    // ====== 从 settings 读取可配置参数 ======
    // Runtime config from DB (fallback to defaults)
    const _rtConfig = await (async () => { try { const r = await getSetting('advanced_config'); return r ? { ...DEFAULT_RUNTIME_CONFIG, ...JSON.parse(r) } : DEFAULT_RUNTIME_CONFIG; } catch { return DEFAULT_RUNTIME_CONFIG; } })();
    let maxSteps = _rtConfig.max_steps;
...
// 兜底温度从DB读取
    ...
    // ====== 读取超时、重试和模型生成参数 ======
    let timeoutTotalMs = _rtConfig.timeout_total_ms;
    let timeoutStepMs = _rtConfig.timeout_step_ms;
    let maxRetries = _rtConfig.max_retries;
    ...
    // DEFAULT_PROVIDER_MAX_TOKENS imported from config-defaults;
    ...
    // === Auto-inject cross-session semantic memory ===
    let crossMemorySection = '';
    try {
      if (typeof lastUserMsg === 'string' && lastUserMsg.trim().length > 5) {
        const { generateEmbedding } = await import('@/lib/rag');
        const queryEmbedding = await generateEmbedding(lastUserMsg.slice(0, 500));
        if (queryEmbedding) {
          const vectorStr = '[' + queryEmbedding.join(',') + ']';
          const memRows = await (await import('@/lib/db')).query(
            `SELECT content, summary, memory_type, importance,
    ...
    const convertDocumentTool = {
      convert_document: tool({
        description: '将Markdown内容转换为PDF或DOCX文档。用于生成可下载的报告、方案文档等。',
        parameters: z.object({
          content: z.string().describe('Markdown格式的内容'),
          format: z.enum(['pdf', 'docx']).describe('输出格式: pdf 或 docx'),
          title: z.string().optional().describe('文档标题（用于文件名）'),
        }),
        execute: async ({ content, format, title }: any) => {
          try {
            const { filePath, fileName } = convertMarkdown(content, format, title);
            const fileBuffer = readConvertedFile(filePath);
            const base64 = fileBuffer.toString('base64');
            cleanupConvertedFile(filePath);
            return { success: true, fileName, format, size: fileBuffer.length, base64Data: base64 };
          } catch (e: any) {
    ...
    activeTools.send_message_to_subagent = tool({
      description: '向已完成的子代理任务发送后续消息并获取回复。当子代理返回结果后你需要追问或补充信息时使用。',
      inputSchema: z.object({
        task_id: z.string().describe('子代理任务ID（从 delegate_task 或 plan_and_execute 返回中获取）'),
        message: z.string().describe('要发送的后续消息'),
      }),
      execute: async ({ task_id, message }) => {
        try {
          const sessionId = convId || 'default';
          await sendToSubAgent(sessionId, task_id, message);
          // Re-trigger the sub-agent to process the message
          const responses = await getSubAgentResponses(sessionId, task_id);
          if (responses.length > 0) {
            return 'Sub-agent response: ' + responses.join('\n');
          }
          return 'Message sent to sub-agent. Response will be available after re-processing.';
        } catch (e: any) {
...
定时任务需要配置cron触发器才会自动执行。触发URL: /api/cron?secret=ai-platform-cron-2026`;
    ...
    const toolNameCn: Record<string, string> = await (async () => { try { const r = await getSetting('tool_name_zh'); return r ? { ...DEFAULT_TOOL_NAME_ZH, ...JSON.parse(r) } : DEFAULT_TOOL_NAME_ZH; } catch { return DEFAULT_TOOL_NAME_ZH; } })();;
    // P1: Load tool safety tiers from DB
    const safetyTiers = await loadToolSafetyTiers(getSetting);
    const sshSafeCommands = await loadSshSafeCommands(getSetting);
    // Build auto-approve set from tiers
    const safeToolsSet = new Set(Object.entries(safetyTiers).filter(([, v]) => v.tier === 'safe').map(([k]) => k));
    const guardedToolsSet = new Set(Object.entries(safetyTiers).filter(([, v]) => v.tier === 'guarded').map(([k]) => k));
    const agentConfig: any = {
      ...
      toolApproval: {
        ...
        build_project: guardedToolsSet.has('build_project') ? 'user-approval' : 'approved',
        deploy_service: guardedToolsSet.has('deploy_service') ? 'user-approval' : 'approved',
        git_commit: guardedToolsSet.has('git_commit') ? 'user-approval' : 'approved',
        // P1: Safe tier tools — auto-approve (managed via tool_safety_tiers in admin)
        ssh_read_file: safeToolsSet.has('ssh_read_file') ? 'approved' : 'user-approval',
      ...
      prepareStep: async ({ messages, stepNumber }: any) => {
        ...
        if (messages.length > 0 && stepNumber > 1) {
          try {
            ...
            if (lastAssistant) {
              const parts = lastAssistant.parts || lastAssistant.content || [];
              const toolResults = Array.isArray(parts) ? parts.filter((p: any) => p.type === 'tool-invocation' || p.type?.startsWith('tool-')) : [];
              for (const tr of toolResults) {
                const tName = tr.toolName || tr.name || '';
                const tState = tr.state || '';
                const tOutput = String(tr.output || tr.result || '');
                if (['build_project', 'deploy_service'].includes(tName) && (tState.includes('error') || tOutput.includes('失败') || tOutput.includes('error') || tOutput.includes('Error'))) {
                  // Force the agent to address the failure before proceeding
                  messages.push({ role: 'user', content: `[系统强制验证] ${tName} 执行失败。你必须先分析错误原因并修复后重试，不允许跳过此错误继续其他任务。请：1) 分析失败原因 2) 修复问题 3) 重新执行${tName}` });
                  break;
                }
              }
            }
        ...
        if (stepNumber >= 1) {
          ...
          if (lastAsstMsg) {
            ...
            const narratePatterns = ["让我", "我先", "我准备", "我来", "好，让我", "我来分析", "我来查看", "我来搜索", "我来收集", "我来读", "我来看", "首先让我", "先看一下", "先看看", "先来看看", "我来检查", "我来帮你"];
      ...
      onToolExecutionEnd: async ({ toolCall, toolOutput, toolExecutionMs }: any) => {
        ...
        const destructiveTools = ['deleteFile', 'runCommand', 'ssh_execute', 'ssh_write_file', 'deploy', 'deploy_service', 'build_project'];
        if (destructiveTools.includes(toolCall.toolName)) {
          const inputArgs = JSON.stringify((toolCall as any).args || (toolCall as any).input || {})?.slice(0, 500) || '';
          auditLog({
            conversation_id: convId,
            user_id: typeof userId !== 'undefined' ? userId : undefined,
            action: (toolCall.toolName === 'ssh_execute' || toolCall.toolName === 'runCommand') ? 'command_exec' :
                    toolCall.toolName === 'ssh_write_file' ? 'ssh_write' :
                    toolCall.toolName === 'deleteFile' ? 'file_delete' :
                    toolCall.toolName === 'deploy' || toolCall.toolName === 'deploy_service' ? 'deploy' :
                    toolCall.toolName === 'build_project' ? 'build' : 'command_exec',
            target: toolCall.toolName,
            detail: inputArgs,
            model_id: typeof model_id !== 'undefined' ? model_id : undefined,
            success: isSuccess,
        ...
        // P闭环: 产出物自动IM投递
        if (isSuccess && ['generate_document', 'generate_ppt', 'convert_document'].includes(toolCall.toolName)) {
          try {
            const { sendAlert } = await import('@/lib/notification');
            const inputArgs = (toolCall as any).args || (toolCall as any).input || {};
            const title = inputArgs.title || inputArgs.name || '文档';
            await sendAlert({
              title: 'AI编程平台 - 文档已生成',
              content: `${toolNameCn[toolCall.toolName] || toolCall.toolName} 已完成: ${title}`,
              level: 'info',
    ...
    const uiStream = createUIMessageStream({
      ...
      if (lastStreamError && !agentResult) {
        ...
        // P80: Use writer.merge to stream ALL content (text + tool calls + approvals)
        // This fixes the root cause: when model calls a tool that needs approval,
        // textStream is empty but the tool-call info must reach the frontend for approval cards
        let text = '';
        let hasStartedResultStream = false;
        try {
          // Merge the full UI message stream - includes text, tool calls, approval requests, reasoning
          writer.merge(agentResult.toUIMessageStream());
          // Wait for the stream to complete so we can read text and steps for post-processing
          text = await agentResult.text || '';
          hasStartedResultStream = text.length > 0;
          if (hasStartedResultStream) {
            logger.info('[P80] Streamed via merge: ' + text.length + ' chars');
          } else {
            logger.info('[P80] Merged stream completed with no text (likely tool-call/approval)');
          }
        } catch (e: any) {
          logger.warn('[AI] merge stream error: ' + (e?.message || String(e)));
          try { text = await agentResult.text || ''; } catch {}
          hasStartedResultStream = text.length > 0;
        }
        // Capture reasoning from the stream result
        let capturedReasoning = '';
        try {
          const resultReasoning = (agentResult as any).reasoning;
          if (resultReasoning && resultReasoning.length > 0) {
            capturedReasoning = resultReasoning.map((r: any) => r.text || ''  ).join('\n');
          }
        } catch {}
        ...
        try { p79_stepsPeek = await agentResult.steps; } catch {}
        const p79_hasAnyToolCall = p79_stepsPeek.some((s: any) => s.toolCalls && s.toolCalls.length > 0);
        if (p79_isCodingMode && p79_hasNarration && !p79_hasAnyToolCall && text.length < 300) {
          ...
          try {
            const p79_forceResult = await generateText({
              ...
              messages: [
                ...
                { role: 'user' as const, content: '继续执行！你说要做就立刻做，不要再说任何话，直接调用工具执行！' },
              ] as any,
              tools: activeTools,
              maxSteps: 5,
            ...
            let p79_forceText = p79_forceResult.text || '';
            const p79_forceSteps = await p79_forceResult.steps;
            const p79_forceToolResults: string[] = [];
            for (const s of p79_forceSteps) {
              for (const tc of s.toolCalls || []) {
                const tr = s.toolResults?.find((r: any) => r.toolCallId === tc.toolCallId);
                if (tr) {
                  const out = typeof tr.output === 'string' ? tr.output : JSON.stringify(tr.output);
                  p79_forceToolResults.push('[' + tc.toolName + '] ' + out.slice(0, 500));
                }
              }
            }
            if (!p79_forceText && p79_forceToolResults.length > 0) {
              try {
                ...
                p79_forceText = p79_summaryResult.text || '';
              } catch {}
            }
            if (p79_forceText && p79_forceText.length > 5) {
              text = p79_forceText;
              const p79_forceId = 'force-' + Date.now();
              writer.write({ type: 'text-start', id: p79_forceId });
              writer.write({ type: 'text-delta', id: p79_forceId, delta: p79_forceText });
              writer.write({ type: 'text-end', id: p79_forceId });
              logger.info('[P79] Force execution succeeded: ' + p79_forceText.length + ' chars, tools: ' + p79_forceToolResults.length);
            }
          } catch (p79_err: any) {
            logger.warn('[P79] Force execution failed: ' + (p79_err?.message || String(p79_err)));
          }
        }
          ...
          // 工具结果只是中间observation，不能作为最终答案直接上屏
            ...
            if (reportToolResults.length > 0) {
              ...
              try {
                ...
                // P76 fix: AI SDK v7 forbids prompt+messages together, use messages only
                const followupMessages = [...chatMessages.slice(-8), { role: 'user' as const, content: '请基于刚才的工具执行结果，用自然语言总结：\n1. 你做了什么\n2. 结果如何\n3. 有什么发现或建议\n\n工具执行结果：\n' + toolResultContext + '\n\n请直接回复，不要重复工具原始输出，要有人类可读的解释和分析。' }];
                ...
                if (followupText.length > 20) {
                  ...
                  // P81b: Check if follow-up text is a structured report -> add REPORT_CARD
                  let p81b_savedContent = '';
                  if (followupText.length >= 1500) {
                    const headerCount = (followupText.match(/^#{1,3} \S/gm) || []).length;
                    if (headerCount >= 3) {
                      const reportTitle = (followupText.match(/^# (.+)$/m)?.[1] || followupText.match(/^## (.+)$/m)?.[1] || '\u5206\u6790\u62a5\u544a').slice(0, 50);
                      const reportId = 'report_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
                      try {
                        const { run: runArtifact } = await import('@/lib/db');
                        await runArtifact('INSERT INTO generated_artifacts (id, type, title, content, created_at) VALUES ($1, $2, $3, $4, NOW())', [reportId, 'markdown', reportTitle, followupText]);
                      } catch (e: any) {
                        console.log('[P81b] Failed to save report artifact: ' + (e?.message || String(e)));
                      }
                      const cardContent = '<!--REPORT_CARD\n' + reportTitle + '\n' + reportId + '\n-->\u70b9\u51fb\u67e5\u770b\u5b8c\u6574\u62a5\u544a';
                      const cardId = 'report-card-' + Date.now();
                      writer.write({ type: 'text-start', id: cardId });
                      writer.write({ type: 'text-delta', id: cardId, delta: '\n\n' + cardContent });
                      writer.write({ type: 'text-end', id: cardId });
                      p81b_savedContent = followupText + '\n\n' + cardContent + '\n<!--EXEC_LOG\n' + execLog + '\n-->';
                      console.log('[P81b] Report card created in follow-up: ' + reportId + ' (' + followupText.length + ' chars)');
                    }
                  }
                  if (!p81b_savedContent) {
                    p81b_savedContent = followupText + '\n<!--EXEC_LOG\n' + execLog + '\n-->';
                  }
                  await createMessage(convId!, 'assistant', p81b_savedContent, model_id, capturedReasoning || null);
      ...
      onError: (error) => {
        ...
        // Add routing info header for debugging
    const responseHeaders: Record<string, string> = {};
    if (routingResult) {
      responseHeaders['X-Model-Route'] = routingResult.routingReason;
    }
    return createUIMessageStreamResponse({ stream: uiStream, headers: responseHeaders });
  } catch (error: any) {
